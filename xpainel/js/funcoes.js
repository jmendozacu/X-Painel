function loadingX(el)
{
	...
	if(el)
	{
		eval('<?=JQUERY?>(\''+el+'\').html(\'<img class="loadingImgX" style="max-width: 32px !important;max-height: 32px !important;" src="'+HTTP+'/xpainel/imagens/loading.gif" /> \');');
	}
	else
	{
		<?=JQUERY?>('#loadingX').fadeIn();
	}
}
function loadedX(el)
{

	<?=JQUERY?>('#loadingX').fadeOut();
}
...
{
	<?=JQUERY?>('#produtoQuantidade').html('');
	<?=JQUERY?>( "#caracteristicaSecundaria").html('');
	...
	<?=JQUERY?>( "#caracteristicaSecundaria").html('<?=U::getLoading()?>');
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=getCaracteristicasSecundarias&caracteristicaPrimaria="+caracteristicaPrimaria+"&produto="+produto, function( data ) {
		<?=JQUERY?>( "#caracteristicaSecundaria").html( data );
	});
}
...
function getEstoque(produto_id, caracteristicasPrimarias, caracteristicasSecundarias)
{
	...
	<?=JQUERY?>( "#estoque").html('<img class="loadingImgX" style="max-width: 32px !important;max-height: 32px !important;" src="'+HTTP+'/xpainel/imagens/loading.gif" />');
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=getEstoque&produto_id="+produto_id+"&caracteristicasPrimarias="+caracteristicasPrimarias+"&caracteristicasSecundarias="+caracteristicasSecundarias, function( data ) {
		<?=JQUERY?>( "#estoque").html( data );
	});
}
function openModalX(contexto)
{
    ...
    <?=JQUERY?>.get(HTTP+'/xpainel/lib/ajax.php?function=modalContents&context='+contexto, function(data) {
      <?=JQUERY?>('#modalXbody' ).html(data);
      <?=JQUERY?>('#modalX').modal({show:true});
    });
}
...
{
	<?=JQUERY?>( "#produtoDisponibilidade").html('<img class="loadingImgX" style="max-width: 32px !important;max-height: 32px !important;" src="'+HTTP+'/xpainel/imagens/loading.gif" />');
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=getEstoqueProduto&produto="+produto, function( data ) {
		<?=JQUERY?>( "#produtoDisponibilidade").html( data );
	});
}
function getEstoqueDisponivel(estoque_grade_id)
{
	<?=JQUERY?>( "#produtoQuantidade").html('<img class="loadingImgX" style="max-width: 32px !important;max-height: 32px !important;" src="'+HTTP+'/xpainel/imagens/loading.gif" />');
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=getEstoqueDisponivel&estoque_grade_id="+estoque_grade_id, function( data ) {
		<?=JQUERY?>( "#produtoQuantidade").html( data );
	});
}
function paginacaoAjaxFotos(pagina)
{
	<?=JQUERY?>('.btpaginacaoAjax').fadeOut();
	loadingX();
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=ajaxFotos&pag="+pagina, function( data ) {
		<?=JQUERY?>( ".containerBtpaginacaoAjax").append( data );
	});
	loadedX();
}
...
function setQtd(acao,indice)
{
	...
	var inputQtd = <?=JQUERY?>('#produto_quantidade'+indice);
	...
	{
		loadingX();
		<?=JQUERY?>('#linhaCarrinho'+indice).submit();
	}
}
...
{
	loadingX();
	<?=JQUERY?>('.hidden'+id).fadeOut( "slow", function() {
		<?=JQUERY?>('#cartajaxbtcart'+id).fadeIn( "slow", function() {
...
{
	console.log(v);
	<?=JQUERY?>('#valorcarrinhosuspenso').html(v);
	loadedX();
}
...
function returnFretes(cep)
{
	...
	console.log(url);
	var retorno = <?=JQUERY?>.get(url, function() {
		 loadingX('#fretes');
	 })
	  .done(function(data) {
	  	<?=JQUERY?>('#fretes').html(data);
	  })
	  .fail(function() {
	   return false;
	  })
...
{
	loadingX('#gateways');
	var retorno = <?=JQUERY?>.get('xpainel/lib/ajax.php?function=getGateways'+noCache(), function() {
	 })
	  .done(function(data) {
	  	<?=JQUERY?>('#gateways').html(data);
	  })
	  .fail(function() {
	   return false;
	  })
...
function setSessionEndereco(elemento)
{
	...
	var validacao = '';
	<?=JQUERY?>.post( url, <?=JQUERY?>(elemento).serialize())
	  .done(function( data ) {
...
function submitAjax(formulario, url)
{
	...
	<?=JQUERY?>.post( url, <?=JQUERY?>(formulario).serialize())
	  .done(function( data ) {
	  	console.log(data);
	  	<?=JQUERY?>('#loadingX').html(data);
  	});
}
function setFrete(valor)
{
	<?=JQUERY?>("input[name=valor_frete]").val(valor);
}
...
function autoCep()
{
	...
	loadingX();
	<?=JQUERY?>.getJSON( 'xpainel/lib/ajax.php?function=getEndereco&cep='+cep+noCache(), function( data ) {
		console.log(data);
		...
		{
			<?=JQUERY?>('#complemento').val(data['clientes_complemento']);
		}
		...
		{
			<?=JQUERY?>('#destinatario').val(data['clientes_destinatario']);
		}
		...
		{
			<?=JQUERY?>('#numero').val(data['clientes_numero']);
		}
		else
		{
			<?=JQUERY?>('#numero').focus();
...
{
	url = typeof url !== 'undefined' ? url : HTTP;
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=addClickFaq&faq_id="+id, function( data ) {
		goHome(url);
	});
}
...
{
	setTimeout(function(){
		<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=addAcessoPaginaPouso&id="+id, function( data ) {
...
function ancorar(lk,add)
{
    ...
    <?=JQUERY?>('html,body').animate({scrollTop: <?=JQUERY?>('#'+lk).offset().top+adicional},'slow');
}

function setIdioma(idioma)
{
	<?=JQUERY?>.get( HTTP+"/xpainel/lib/ajax.php?function=setIdioma&idioma="+idioma, function( data ) {
